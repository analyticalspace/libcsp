 ...
 * @param f Little endian byte order float
 * @return Float in host order
 */
float csp_letohflt(float f);
...
/**
 * Convert float host byte order to little endian byte order
 * @param f Float in host order
 * @return Float in little endian byte order
 */
float csp_htoleflt(float f);
