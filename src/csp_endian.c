...
/* Convert float from little endian byte order tp host byte order */
inline float CSP_COMPILER_CONST csp_letohflt(float f) {
#ifdef CSP_LITTLE_ENDIAN
    return f;
#else
    union v {
...
/* Convert float from host byte order to little endian byte order */
inline float CSP_COMPILER_CONST csp_htoleflt(float f) {
    return csp_letohflt(f);
}
