...
static CSP_DEFINE_TASK(csp_bridge)
{
    ...
    while (1)
    {
        ...
        if (input.interface == if_a) { ifout = if_b; }
        else                         { ifout = if_a; }
...
int csp_bridge_start(unsigned int task_stack_size, unsigned int task_priority, csp_iface_t * _if_a, csp_iface_t * _if_b)
{
    ...
    if (0 != csp_thread_create(csp_bridge, "BRIDGE", task_stack_size, NULL, task_priority, &handle))
    {
        csp_log_error("csp_bridge: Failed to start task");
