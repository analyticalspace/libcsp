...
unsigned int csp_global_buf_size = 0;
unsigned int csp_global_buf_count = 0;

CSP_DEFINE_CRITICAL(csp_critical_lock);

int csp_buffer_init(unsigned int buf_count, unsigned int buf_size)
{
    unsigned int i;
    csp_skbf_t * buf;
    ...
    /* set the globals */
    csp_global_buf_size = buf_size;
    csp_global_buf_count = buf_count;
